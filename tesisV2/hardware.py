...
class HardwareManager:
    ...
    def _configurar_salida(self, gpio: int, activa_bajo: bool, encendido: bool) -> None:
        # Fijamos el nivel seguro ANTES de conmutar el pin a salida, para que
        # no haya un pulso espurio al relE en el instante del GPIO.setup.
        nivel_seguro = self._valor_salida(activa_bajo, encendido)
        GPIO.setup(gpio, GPIO.OUT, initial=nivel_seguro)
        GPIO.output(gpio, nivel_seguro)
    ...
    def cleanup(self) -> None:
        ...
        # CORREGIDO: antes limpiaba solo [5, 19, 26] (lista arbitraria e incompleta).
        # GPIO.cleanup() sin argumentos libera todos los pines que este proceso configuro.
        GPIO.cleanup()
