...
def _validar_macetas(config: SystemConfig) -> None:
    for nombre_maceta, maceta in config.macetas.items():
        if not (0 <= maceta.umbral_humedad_suelo_raw <= 255):
            raise ValueError(f"{nombre_maceta}: umbral_humedad_suelo_raw fuera de rango (0-255)")

        # CORREGIDO: el campo es umbral_humedad_ambiente_pct y su rango es 0-100 (porcentaje)
        if not (0 <= maceta.umbral_humedad_ambiente_pct <= 100):
            raise ValueError(f"{nombre_maceta}: umbral_humedad_ambiente_pct fuera de rango (0-100)")
